...
function AddSoundtrackForm() {
    ...
    function handleChange(event) {
        ...
        let value = trim(field.value);

        if (name.startsWith("track.")) {
            const trackField = name.split(".")[1];
            setFormData((prevData) => ({
                ...prevData,
                track: {
                    ...prevData.track,
 ...
 function isFormValid() {
     ...
     if (!isLength(formData.image, { min: 3, max: 100 })) {
         newErrors.image = "L'image doit contenir entre 3 et 100 caractères.";
     }

     if (!isInt(formData.year, { min: 1900, max: new Date().getFullYear() })) {
            newErrors.year =
